...
const Demo = () => {
  ...
  const quadrantData = {
    1: { title: 'Urgente e Importante', icon: AlertTriangle, color: 'bg-red-500', textColor: 'text-red-500', gradient: 'from-red-50/80 via-red-50/50 to-red-50/30' },
    2: { title: 'Importante, Não Urgente', icon: CheckSquare, color: 'bg-green-500', textColor: 'text-green-500', gradient: 'from-green-50/80 via-green-50/50 to-green-50/30' },
    3: { title: 'Urgente, Não Importante', icon: Clock, color: 'bg-amber-500', textColor: 'text-amber-500', gradient: 'from-amber-50/80 via-amber-50/50 to-amber-50/30' },
    4: { title: 'Nem Urgente, Nem Importante', icon: Trash2, color: 'bg-gray-500', textColor: 'text-gray-500', gradient: 'from-gray-50/80 via-gray-50/50 to-gray-50/30' }
  };
          ...
          {activeTab === 'all' ? (
            ...
            >
              {[1, 2, 3, 4].map(quadrant => {
                ...
                const { title, icon: Icon, color, gradient } = quadrantData[quadrant as 1 | 2 | 3 | 4];
                ...
                {Object.entries(quadrantData).map(([key, data]) => {
                  ...
                  return (
                    <motion.div
                      ...
                      whileTap={{ scale: 0.98 }}
                      className={`border rounded-lg p-3 cursor-pointer transition-all shadow-sm hover:shadow-md ${
...
const TaskCard = ({ 
  ...
  task: Task; 
  quadrantData: { title: string; icon: React.ElementType; color: string; textColor: string; gradient: string; }; 
